...
namespace dicom
{
    ...
    namespace
    {
            ...
            for (int z = zStart; z < zEnd; ++z)
            {
                ...
                for (int y = 0; y < h; ++y)
                {
                    for (int x = 0; x < w; ++x)
                    {
                        ...
                        if (result.labels[idx] != 0)
                            continue;
                        if (!inMask(volume, x, y, z, huMin, huMax))
                            continue;
                        ...
                        while (!stack.empty())
                        {
                            ...
                            for (int dz = -1; dz <= 1; ++dz)
                            {
                                ...
                                if (nlz < 0 || nlz >= slabDepth)
                                    continue; // stay within this slab
                                ...
                                for (int dy = -1; dy <= 1; ++dy)
                                {
                                    ...
                                    for (int dx = -1; dx <= 1; ++dx)
                                    {
                                        ...
                                        if (result.labels[nidx] != 0)
                                            continue;
                                        if (!inMask(volume, nx, ny, nz, huMin, huMax))
                                            continue;
                                               ...
                                               const AnomalyDetector::Thresholds &t)
        {
            ...
            {
                if (comp.voxelCount == 0)
                    continue;
                if (comp.voxelCount < t.minVoxelCount || comp.voxelCount > t.maxVoxelCount)
                    continue;
                                                         ...
                                                         const Thresholds &thresholds)
    {
        ...
        for (const auto &r : ranges)
        {
            ...
            futures.push_back(
                pool.submit([&volume, r, huMin, huMax]
                            { return floodFillLabelRegion(volume, r.first, r.second, huMin, huMax); }));
        }
        ...
        for (size_t i = 0; i + 1 < ranges.size(); ++i)
        {
            ...
            auto idxA = [&](int x, int y) -> size_t
            { return (static_cast<size_t>(localZLast) * h + y) * w + x; };
            auto idxB = [&](int x, int y) -> size_t
