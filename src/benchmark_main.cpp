...
int main(int argc, char* argv[]) {
    ...
    if (!hasAVX2 || forceScalarOnly) {
        ...
        printScalarRow("Gaussian Blur", timeMsAvg([&] { dicom::filters::gaussianBlurScalar(volume, 2.0f); }, iterations));
        printScalarRow("Sobel Edge", timeMsAvg([&] { dicom::filters::sobelEdgeScalar(volume); }, iterations));
        printScalarRow("Histogram Equalize", timeMsAvg([&] { dicom::filters::histogramEqualizeScalar(volume); }, iterations));
        return EXIT_SUCCESS;
    }
    ...
    printRow("Gaussian Blur", timeMsAvg([&] { dicom::filters::gaussianBlurScalar(volume, 2.0f); }, iterations),
              timeMsAvg([&] { dicom::filters::gaussianBlurAVX2(volume, 2.0f); }, iterations));
    printRow("Sobel Edge", timeMsAvg([&] { dicom::filters::sobelEdgeScalar(volume); }, iterations),
              timeMsAvg([&] { dicom::filters::sobelEdgeAVX2(volume); }, iterations));
    printRow("Histogram Equalize", timeMsAvg([&] { dicom::filters::histogramEqualizeScalar(volume); }, iterations),
              timeMsAvg([&] { dicom::filters::histogramEqualizeAVX2(volume); }, iterations));
