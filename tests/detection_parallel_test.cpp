...
namespace
{
    ...
    bool anomaliesMatch(std::vector<dicom::Anomaly> a, std::vector<dicom::Anomaly> b)
    {
        ...
        {
            if (a[i].voxelCount != b[i].voxelCount)
                return false;
            if (a[i].centroid != b[i].centroid)
                return false;
            if (a[i].bboxMin != b[i].bboxMin || a[i].bboxMax != b[i].bboxMax)
                return false;
            if (std::abs(a[i].meanHU - b[i].meanHU) > 1e-6)
                return false;
            if (std::abs(a[i].stddevHU - b[i].stddevHU) > 1e-6)
                return false;
        }
        return true;
    }
